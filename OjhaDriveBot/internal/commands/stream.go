...
func sendExternalLinkNotification(fileID int64, fileName string, fileSize int64, mimeType string, messageID int, hash, streamURL, downloadURL string) {
	webhookURL := strings.TrimSpace(config.ValueOf.LinkWebhookURL)
	if webhookURL == "" {
		return
	}
	...
	if err != nil {
		utils.Logger.Sugar().Errorf("failed to marshal external link payload: %v", err)
		return
	}
	...
	if err != nil {
		utils.Logger.Sugar().Errorf("failed to create external link request: %v", err)
		return
	}
	...
	if err != nil {
		utils.Logger.Sugar().Errorf("failed to send external link request: %v", err)
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		utils.Logger.Sugar().Errorf("external link endpoint returned non-2xx status: %s", resp.Status)
	}
}
...
func sendLink(ctx *ext.Context, u *ext.Update) error {
	...
	downloadLink := link + "&d=true"

	go sendExternalLinkNotification(
		file.ID,
		file.FileName,
		file.FileSize,
		file.MimeType,
