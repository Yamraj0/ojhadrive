...
type config struct {
	...
	// URL to POST stream/download link payload when a file link is generated (optional)
	LinkWebhookURL string `envconfig:"LINK_WEBHOOK_URL"`
}
...
func SetFlagsFromConfig(cmd *cobra.Command) {
	...
	cmd.Flags().String("link-webhook-url", ValueOf.LinkWebhookURL, "URL to POST stream/download link payload when a file link is generated")
}
...
func (c *config) loadConfigFromArgs(log *zap.Logger, cmd *cobra.Command) {
	...
	linkWebhookURL, _ := cmd.Flags().GetString("link-webhook-url")
	if linkWebhookURL != "" {
		os.Setenv("LINK_WEBHOOK_URL", linkWebhookURL)
	}
}
